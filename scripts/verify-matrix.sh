        ...
        linux/arm)
            ...
            if [[ "$command" == dir || "$command" == tasklist ]]; then
                if ! undefined_symbols="$("$nm_bin" -u "$artifact")"; then
                    echo "cannot inspect Linux/arm directory imports: $artifact with $nm_bin" >&2
                    failures=$((failures + 1))
                else
                    if ! awk '$NF == "readdir64" { found = 1 } END { exit found ? 0 : 1 }' <<<"$undefined_symbols"; then
                        echo "Linux/arm directory enumeration does not import readdir64: $artifact" >&2
                        failures=$((failures + 1))
                    fi
                    if awk '$NF == "readdir" { found = 1 } END { exit found ? 0 : 1 }' <<<"$undefined_symbols"; then
                        echo "Linux/arm directory enumeration imports legacy readdir: $artifact" >&2
                        failures=$((failures + 1))
                    fi
                fi
            fi
